...
const Toast: React.FC<ToastProps> = ({ message, visible }) => (
  <div
    aria-live="polite"
    className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 px-5 py-3 rounded-2xl bg-slate-800 border border-slate-600 shadow-2xl text-sm font-medium text-slate-100 transition-all duration-300 ${
...
const BalanceBadge: React.FC<BalanceBadgeProps> = ({ sumOfShares, billTotal, currency }) => {
  ...
  if (isBalanced) {
    return (
      <div className="flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 shadow-lg shadow-emerald-900/20">
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-300 shadow-lg shadow-amber-900/20">
...
const HeaderSummary: React.FC<HeaderSummaryProps> = ({ billData, currency, participantCount }) => {
  ...
  const stats = [
    { label: 'Grand Total', value: formatCurrency(grandTotal, currency), highlight: true, color: 'text-white' },
    { label: 'Subtotal', value: formatCurrency(subtotal, currency), highlight: false, color: 'text-slate-200' },
    ...(tax > 0 ? [{ label: 'Tax', value: formatCurrency(tax, currency), highlight: false, color: 'text-indigo-300' }] : []),
    ...(serviceCharge > 0
      ? [{ label: 'Service Charge', value: formatCurrency(serviceCharge, currency), highlight: false, color: 'text-indigo-300' }]
      : []),
    ...(tip > 0 ? [{ label: 'Tip', value: formatCurrency(tip, currency), highlight: false, color: 'text-purple-300' }] : []),
    ...(discount > 0
      ? [{ label: 'Discount', value: `−${formatCurrency(discount, currency)}`, highlight: false, color: 'text-emerald-400' }]
      : []),
    {
...
const PersonCard: React.FC<PersonCardProps> = ({ person, currency }) => {
  ...
  return (
    <div
      className={`rounded-3xl border bg-slate-900/80 backdrop-blur-md shadow-xl flex flex-col overflow-hidden transition-all ${theme.badgeBorder}`}
    >
      {/* ── Card Header ── */}
      <div className="p-5 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-base text-slate-100 truncate">{person.participantName}</h3>
          ...
          <div className="text-right flex-shrink-0 ml-3">
            <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Total to Pay</span>
      ...
      {expanded && (
        <div className="border-t border-slate-800/80 px-5 py-4 space-y-4">
            ...
            <div className="space-y-1.5 max-h-52 overflow-y-auto pr-1">
              {person.itemBreakdowns.map((it, idx) => (
                <div key={idx} className="flex items-start justify-between gap-2 text-xs">
                  <div className="min-w-0">
                    <span className="text-slate-200 font-medium block">{it.itemName}</span>
                    {it.splitCount > 1 && (
          ...
          <div className="space-y-1.5 text-xs border-t border-slate-800/60 pt-3">
            ...
            {taxAndFees > 0 && (
              <div className="flex justify-between text-slate-400">
                ...
                <span className="font-mono text-indigo-300">{formatCurrency(taxAndFees, currency)}</span>
              </div>
            )}
            ...
            {tip > 0 && (
              <div className="flex justify-between text-slate-400">
                <span>+ Tip</span>
                <span className="font-mono text-purple-300">{formatCurrency(tip, currency)}</span>
              </div>
            )}
            ...
            {discount > 0 && (
              <div className="flex justify-between text-slate-400">
                <span>− Discount</span>
                <span className="font-mono text-emerald-400">−{formatCurrency(discount, currency)}</span>
              </div>
            )}

            {/* Final total row */}
            <div className={`pt-2.5 border-t border-slate-700 flex justify-between items-center`}>
              <span className="text-sm font-bold text-slate-100">Total to Pay</span>
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-200">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-slate-500">{breakdowns.length} participants</span>
      </div>
      ...
      {/* ── Bottom CTA Strip ── */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 pb-2 border-t border-slate-800">
